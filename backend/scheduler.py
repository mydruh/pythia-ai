...
class TradingCycle:
    ...
    async def run_stop_loss_check(self) -> None:
        ...
        try:
            async with async_session_factory() as session:
                ...
                for ts in sessions.all():
                    try:
                        await _settle_resolved_positions(session, ts.id, self.executor)
                    except Exception:
    ...
    async def _session_cycle(self, session: AsyncSession, ts: TradingSession) -> None:
        ...
        for market, token, prob in finalists:
            ...
            # Предохранитель «нет мнения»: оценка модели близка к 0.5 = монетка/незнание.
            # Большой edge против крайней цены рынка тогда иллюзорен — рынок видит то,
            # чего не видит модель (свежие данные / knowledge cutoff). Не открываем
            # позицию. Анализ уже записан (Brier/аудит), в UI лот покажется как
            # «заблокирован». Не трогает фильтр/сайзинг/лимиты/settlement.
            if not self.analyzer.has_conviction(result):
                logger.info(
                    "session=%d skip '%s': модель без убеждённости (my_prob=%.2f, edge=%+.2f) — нет реального мнения",
                    ts.id, market.question[:40], result.prob, edge,
                )
                continue
