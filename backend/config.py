...
class Settings(BaseSettings):
    ...
    # Ниже минимума — мёртвая зона: почти нет ликвидности, худший favorite-longshot bias.
    # Выше максимума — отсекаем фаворитов, фокусируемся на лонгшот-конце рынков.
    # ЭКСПЕРИМЕНТ (29 июня): сужено до 0.10–0.35 — смотрим работу на рискованных
    # (низковероятных) лотах. Тезис юзера: 1–2 победы из 5 окупают 3–4 проигрыша за
    # счёт большого payout. ВНИМАНИЕ: лонгшоты исторически переоценены → ставка НА них
    # обычно −EV, edge в основном в BUY_NO (фейд переоценённого лонгшота). Направление
    # выбирает модель; смотрим /accuracy по направлениям. Откат: вернуть 0.30 / 0.85.
    risk_min_market_prob: float = 0.10
    risk_max_market_prob: float = 0.35
    # Чёрный список паттернов в ВОПРОСЕ рынка (case-insensitive substring). Рынок,
    # чей вопрос содержит любой паттерн, НЕ берётся в анализ (отсекается до LLM-вызова
    # — экономит токены и не плодит NEUTRAL-следы). Цель — «новелти-шум»: рынки, где
    # у модели физически нет edge (что скажет комментатор, счётчики твитов и т.п.).
    # На данных бота 3 эти рынки дали 0 побед из 6 (−9.12 USDC). Реальный спорт
    # (кто победит, тоталы, спреды) НЕ трогаем. Переопределяется через env как JSON.
    market_blacklist_patterns: list[str] = [
        "announcer",   # «сколько раз комментатор скажет Foul/Corner/Oh my God» — шум
        "tweets",      # счётчики твитов «post 40-64 tweets» — шум
    ]
